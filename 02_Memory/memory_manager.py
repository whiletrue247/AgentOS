...
v5.1 新增 (Sprint 2)：
  - access_count 追蹤：每次搜尋命中自動增加存取計數
  - 記憶品質衡量式：score = importance × (1 + log(access_count)) × exp(-λt)
  - 高存取頻率的記憶衰減更慢，低頻率的淡化更快
"""
...
class MemoryManager:
    ...
    async def search(
        ...
        for mem in results:
            ...
            try:
                await self._provider.write(mem)  # 回寫 access_count
            except Exception:
                pass  # 存取計數更新失敗不影響搜尋結果
    ...
    async def run_decay_cycle(
        ...
        v5.1 改進公式 (Sprint 2)：
          quality_score = importance × (1 + log2(1 + access_count)) × 0.5^(days / half_life)
        
        access_count 高的記憶衰減更慢（表示常用）。
        quality_score 低於 min_importance 的記憶會被刪除。
        ...
        try:
            ...
            for mem in candidates:
                ...
                # Sprint 2: 加入 access_count 權重
                access_count = mem.metadata.get("access_count", 0)
                access_boost = 1.0 + math.log2(1.0 + access_count)

                # 計算衰減後的品質分數
                time_decay = math.pow(0.5, days_age / half_life_days)
                quality_score = mem.importance * access_boost * time_decay

                if quality_score < min_importance:
                    await self._provider.delete(mem.memory_id)
                    forgotten += 1
                    logger.debug(
                        f"🗑️ 記憶衰減遺忘: {mem.content[:30]}... "
                        f"(importance={mem.importance:.2f}, access={access_count}, "
