...
async def boot() -> None:
    ...
    # ── 註冊 signal handler (graceful shutdown) ──
    loop = asyncio.get_running_loop()
    shutdown_event = asyncio.Event()

    def _signal_handler(sig: signal.Signals) -> None:
        logger.info(f"📡 收到信號 {sig.name}，準備 graceful shutdown...")
        shutdown_event.set()
        # 取消所有 running tasks（讓 CancelledError 傳播）
    ...
    try:
        ...
        if use_terminal:
            ...
            while True:
                try:
                    ...
                    logger.info("📡 Terminal 收到取消信號，準備退出...")
                    break
                ...
                # 呼叫 Engine（含例外恢復，不讓單次錯誤崩潰整個 loop）
                print()
                try:
                    ...
                    logger.info("📡 Engine 處理中收到取消信號")
                    break
                except Exception as e:
                    logger.error(f"❌ Engine 處理失敗: {e}", exc_info=True)
                    reply = f"⚠️ 處理您的請求時發生錯誤，請重試。({type(e).__name__}: {e})"
                ...
                try:
                    ...
                    logger.warning(f"⚠️ Cost Guard 同步失敗: {e}")
        ...
        logger.info("📡 收到取消信號，開始 graceful shutdown...")
    except Exception as e:
        logger.error(f"❌ 未預期的嚴重錯誤: {e}", exc_info=True)
    finally:
        logger.info("🔄 正在執行 graceful shutdown...")
        await _shutdown(
...
def main():
    try:
        ...
        # signal handler 已在 boot() 中處理，這裡僅為最外層兜底
        ...
        logger.critical(f"💀 致命錯誤: {e}", exc_info=True)
        sys.exit(1)
